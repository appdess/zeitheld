        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Button(account.signedIn || preferences.cloudVoiceMode == .parentKey ? copy("Auswahl bestätigen und speichern", "Confirm and save choices")
...
struct ParentPrivacySummary: View {
    ...
    var body: some View {
        ...
        Text(copy("Eigener API-Key: Wenn du diesen Zugang wählst, gehen Gespräche und Heldenanfragen direkt an OpenAI. Unser Backend erhält weder deinen Key noch diese Anfragen. Die Bestätigung bleibt lokal, solange du sie nicht mit einem Elternkonto verknüpfst. OpenAI rechnet die Nutzung separat ab.", "Own API key: when you select this access method, conversations and hero requests go directly to OpenAI. Our backend receives neither your key nor these requests. Confirmation remains local unless you link it to a parent account. OpenAI bills the usage separately."))
